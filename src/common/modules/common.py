...
class VGG11Slim(nn.Module):
    ...
    def __init__(
        ...
        if dropout:
            ...
            self.model.features = nn.Sequential(*feats)
        ...
        self.model.classifier = nn.Linear(in_features, hiddim)
...
class CLIPVisionEncoder(nn.Module):
    """
    用 CLIP 视觉塔（clip-vit-base-patch32）替代 VGG11Slim。
    - patch=True  -> 输出 (B, num_patches, hidden_dim)
    - patch=False -> 输出 (B, hidden_dim)
    """
    def __init__(self, hidden_dim, model_name="openai/clip-vit-base-patch32",
                 patch=True, num_patches=16, freeze_vision=True, local_only=True):
        super().__init__()
        self.patch = patch
        ...
        width = self.vision.config.hidden_size  # 768 for ViT-B
        if self.patch:
            self.token_pool = nn.AdaptiveAvgPool1d(num_patches)
            self.proj = nn.Linear(width, hidden_dim)
        else:
            self.proj = nn.Linear(width, hidden_dim)
    ...
    def forward(self, pixel_values):
        ...
        # last_hidden_state: (B, 1+49, 768) —— 含 CLS
        if self.patch:
            tokens = out.last_hidden_state[:, 1:, :]                     # (B, 49, 768)
            tokens = self.token_pool(tokens.transpose(1, 2)).transpose(1, 2)  # -> (B, num_patches, 768)
            return self.proj(tokens)                                     # (B, num_patches, hidden_dim)
        else:
