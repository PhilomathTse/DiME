...
class MultiModalDataset(Dataset):
    def __init__(
        ...
        self.data = {}
        for modality, data in self.data_dict.items():
            if "modality" in modality:
                continue
            if isinstance(data, list) and len(data) > 0 and isinstance(data[0], dict):
                self.data[modality] = [data[i] for i in ids]
            else:
                self.data[modality] = np.array(data)[ids]
        ...
        for modality, data in self.data.items():
            ...
            if isinstance(value, dict):
                sample_data[modality] = {
                    k: np.nan_to_num(v) for k, v in value.items()
                }
            else:
                sample_data[modality] = np.nan_to_num(value)
                if modality == "image":
                    sample_data[modality] = self.process_2d_to_3d(
                        data, idx, self.masks, self.transforms
                    )
...
#@Zy
def _stack_modality(modality_samples):
    if isinstance(modality_samples[0], dict):
        return {
            k: torch.tensor(np.stack([ms[k] for ms in modality_samples]), dtype=torch.long)
            for k in modality_samples[0].keys()
        }
    else:
        return torch.tensor(np.stack(modality_samples), dtype=torch.float32)
...
def collate_fn(batch):
    ...
    collated_data = {
        ...
        # )
        # for modality in modalities#@Zy
        modality: _stack_modality([d[modality] for d in data]) for modality in modalities
    }
