...
def plot_interaction_loss_curves(
    ...
    plt.savefig(
        str(save_dir)
        + f"/{args.data}_{args.target}__epochs_{args.train_epochs}_{args.modality}_seed_{args.seed}-{now.strftime('%Y-%m-%d_%H:%M:%S')}.pdf",
        dpi=300,
    )
...
def plot_total_loss_curves(
    ...
    plt.savefig(
        str(save_dir)
        + f"/{args.data}_{args.target}_epochs_{args.train_epochs}_{args.modality}_seed_{args.seed}-{now.strftime('%Y-%m-%d_%H:%M:%S')}.pdf",
        dpi=300,
    )
...
def visualize_sample_weights(
    ...
    n_mod = len(args.modality)
    n_exp = weights_array.shape[1]
    labels = [f"Uniqueness {i+1}" for i in range(n_mod)]
    if n_exp == n_mod + 2:
        labels += ["Synergy", "Redundancy"]
    elif n_exp == n_mod + 1:
        labels += ["Redundancy"]
    else:
        labels = [f"Expert {i+1}" for i in range(n_exp)]
    
    data_cols = [weights_array[:, i] for i in range(n_exp)]
    vp = plt.violinplot(data_cols, showmeans=True)
    for idx, body in enumerate(vp["bodies"]):
        body.set_facecolor(COLORS[idx])
        # body.set_alpha(0.7)  # Set transparency
    # plt.xticks(range(1, n_exp + 1), labels)
    plt.xticks(list(range(1, n_exp + 1)), labels, rotation=0)
    #@Zy
...
def plot_loss_curves_MI_shared(
    ...
    if plotting_total_losses is None: plotting_total_losses = {}
    if plotting_interaction_losses is None: plotting_interaction_losses = {}
    ...
    # for name, losses in plotting_total_losses.items():
    #     plt.plot(epochs, losses, label=name) #@zy
    ...
    if is_latent:
        ...
        plt.savefig(
            f"./figures/loss_curves/{args.data}/interaction_shared/total_loss/{args.target}_{args.modality}_SP_{args.fusion_sparse}_GT_{args.gate}_MR_{args.weighting_method}_epoch_{args.train_epochs}.pdf",
            dpi=300,
        )
    else:
        Path(f"./figures/loss_curves/{args.data}/interaction_shared/total_loss").mkdir(
    ...
    if is_latent:
        ...
        plt.savefig(
            f"./figures/loss_curves/{args.data}/interaction_shared/interaction_loss/{args.target}_{args.modality}_SP_{args.fusion_sparse}_GT_{args.gate}_MR_{args.weighting_method}_epoch_{args.train_epochs}.pdf",
            dpi=300,
        )
    else:
        Path(
...
def plot_loss_curves_MI_shared_baselines(
    ...
    # epochs = range(1, args.train_epochs + 1)#@zy
    ...
    if is_latent:
        ...
        plt.savefig(
            f"./figures/loss_curves/{args.data}/{args.model}/interaction_shared/total_loss/{args.target}_{args.modality}_SP_{args.fusion_sparse}_GT_{args.gate}_MR_{args.weighting_method}_epoch_{args.train_epochs}.pdf",
            dpi=300,
        )
    else:
        Path(
        ...
        plt.savefig(
            f"./figures/loss_curves/{args.data}/{args.model}/interaction_shared/total_loss/{args.target}_{args.modality}_MR_{args.weighting_method}_epoch_{args.train_epochs}.pdf",
            dpi=300,
        )
    ...
    if is_latent:
        ...
        plt.savefig(
            f"./figures/loss_curves/{args.data}/{args.model}/interaction_shared/interaction_loss/{args.target}_{args.modality}_MR_{args.weighting_method}_epoch_{args.train_epochs}.pdf",
            dpi=300,
        )
    else:
        Path(
