...
class InteractionExpert(nn.Module):
    ...
    def forward_with_replacement(self, inputs, replace_index,replacement_tensor=None):#@Zy
        """
        ...
        #@Zy
        return self._forward_with_replacement(inputs, replace_index=replace_index, replacement_tensor=replacement_tensor)
    ...
    def _forward_with_replacement(self, inputs, replace_index=None, replacement_tensor=None): 
        ...
        if replace_index is not None:
            if replacement_tensor is None:
                repl = torch.randn_like(inputs[replace_index])
            else:
                repl = replacement_tensor.to(
                    device=inputs[replace_index].device, dtype=inputs[replace_index].dtype
                )
                assert repl.shape == inputs[replace_index].shape, \
                    f"replacement shape {repl.shape} != input shape {inputs[replace_index].shape}"
            inputs = inputs[:replace_index] + [repl] + inputs[replace_index + 1:]
    ...
    def forward_multiple(self, inputs,replacement_tensors=None):#@Zy
        """
        ...
        #         #@Zy
        #         repl = None if replacement_tensors is None else replacement_tensors.get(i, None)
        ...
        if self.fusion_sparse:
            ...
            out, gate_loss = self.forward(inputs)  
            outputs.append(out)
            gate_losses.append(gate_loss)

            for i in range(len(inputs)):
                repl = None if replacement_tensors is None else replacement_tensors.get(i, None)
                out, gate_loss = self.forward_with_replacement(inputs, replace_index=i, replacement_tensor=repl)
                outputs.append(out)
                gate_losses.append(gate_loss)
...
class DiME(nn.Module):
    ...
    def uniqueness_loss_single(self, anchor, pos, neg):
        triplet_loss = nn.TripletMarginLoss(margin=1.0, p=2, eps=1e-7)#1.0
    ...
    def forward(self, inputs,replacement_tensors=None):#@Zy
        ...
        # else:
        #     for expert in self.interaction_experts:
        #         expert_outputs.append(expert.forward_multiple(inputs))#@Zy
        expert_outputs = []
        if self.fusion_sparse:
            expert_gate_losses = []
            for expert in self.interaction_experts:
                expert_output, expert_gate_loss = expert.forward_multiple(
                    inputs, replacement_tensors=replacement_tensors
                )
                expert_outputs.append(expert_output)
                expert_gate_losses.append(expert_gate_loss)
        else:
            for expert in self.interaction_experts:
                expert_outputs.append(expert.forward_multiple(
                    inputs, replacement_tensors=replacement_tensors
                ))
        ...
        # interaction_losses = uniqueness_losses + [synergy_loss] + [redundancy_loss]#@zy
        interaction_losses = uniqueness_losses + [redundancy_loss]#@Zy
        # interaction_losses = uniqueness_losses
        ...
        if interaction_weights.size(1) == self.num_modalities + 1:
            w = interaction_weights.clone()
            w[:, -1] = 0.0
            w = w / (w.sum(dim=1, keepdim=True) + 1e-8)
        else:
            w = interaction_weights#@Zy
